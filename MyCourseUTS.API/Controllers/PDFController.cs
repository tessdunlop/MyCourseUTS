...
namespace MyCourseUTS.API.Controllers
{
    ...
    {
        //http://mycourseuts.azurewebsites.net/Services/api/pdf/getpdf?courseID=C09067&majorID=MAJ03518
        ...
        public void getPDF(string courseID, string majorID = "empty")
        {
            ...
            else
            {
                column = new DataColumn();
                column.DataType = System.Type.GetType("System.String");
                column.ColumnName = "Sorry! There is no template for this course or major.";
                table.Columns.Add(column);
            }
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                if (majorID == "empty")
                {//means that its a course relationship with a major               
                    HttpContext.Current.Response.AddHeader("Content-Disposition", "attachment; filename="+ courseID + ".pdf");
                }
                else
                {//just a course relationship
                    HttpContext.Current.Response.AddHeader("Content-Disposition", "attachment; filename="+ majorID +".pdf");
                }
